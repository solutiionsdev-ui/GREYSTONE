...
> [!tip] The home view is already built
> `src/views/home.tsx` (route `/`) is the AERRA landing page, ported from the
> Figma frame "Concept 3". It is **not** a blank canvas — see [[routing]] for the
> section map. To change it, edit the section component that owns the band and
> follow the *Updating an existing page* note at the bottom of this playbook.
> Scaffold a new route only when the work genuinely belongs on one.
