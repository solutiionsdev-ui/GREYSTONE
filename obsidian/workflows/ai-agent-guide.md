...
> [!tip] Where to start
> The home view (`src/views/home.tsx`, route `/`) is **built** — it is the AERRA
> landing page, ported from Figma. It is no longer the blank canvas the starter
> shipped, so **do not develop new work into it by default**: read [[routing]]
> for its section map, then change the section that owns the area. Preserve
> existing logic and keep diffs minimal ([[new-page]]).
