...
> [!note] `position: fixed` is safe here
> Lenis is constructed without a `wrapper` / `content` pair, so it drives
> **native window scroll** rather than translating a wrapper element. Fixed
> elements therefore behave normally — which is what the fixed site header in
> `src/views/home/site-nav.tsx` relies on. Introducing a transformed wrapper
